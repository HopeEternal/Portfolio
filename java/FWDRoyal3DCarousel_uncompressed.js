...
(function (window){
	...
	var FWDComboBox = function(parent, props_obj){
		...
		this.openMenuHandler = function(){
			...
			self.dispatchEvent(FWDComboBox.OPEN);
		};
...
var FWDComboBoxSelector = function(
		...
		self.setupMainContainers = function(){
			...
			self.arrowN_sdo = new FWDSimpleDisplayObject("div");
			self.arrowN_sdo.screen.style.backgroundImage = "url(" + self.arrowN_str + ")";
			self.arrowS_sdo = new FWDSimpleDisplayObject("div");
		...
		self.destroy = function(){
			...
			TweenMax.killTweensOf(self.text_sdo);
			TweenMax.killTweensOf(self.colorObj);
			self.text_sdo.destroy();
...
(function(window)
{
	...
	{
		FWDR3DCarData.prototype = new FWDEventDispatcher();
	};
...
(function(window)
{
	...
	{
		FWDR3DCarScrollbar.prototype = null;
		FWDR3DCarScrollbar.prototype = new FWDDisplayObject("div");
	};
...
}(window));/* FWDR3DCarSlideshowButton */
(function(window)
{
	...
	{
		FWDR3DCarSlideshowButton.prototype = new FWDDisplayObject("div");
	};
...
(function(window)
{
	var FWDR3DCarThumb = function(id, data, parent)
	{
		...
		this.setupScreen = function()
		{
			...
			if (!self.isMobile && (!data.showThumbnailsHtmlContent || !data.enableHtmlContent))
			{
				self.mainDO.setButtonMode(true);
			}
			
			if (!self.isMobile && !FWDUtils.isIEAndLessThen9 && data.showGradient)
			{
				self.setupGradient();
			}
		...
		{
			if (!self.imageDO || self.isMobile || FWDUtils.isIEAndLessThen9)
				return;
			
			var imgW = self.thumbWidth - self.borderSize * 2;
			var imgH = self.thumbHeight - self.borderSize * 2;
			
			self.reflCanvasDO = new FWDSimpleDisplayObject("canvas");
		...
		this.showThumb3D = function()
		{
			...
			else
			{
				...
				if (self.reflCanvasDO)
				{
					...
					TweenMax.to(self.reflCanvasDO, .8, {alpha:1, ease:Expo.easeInOut});
				}
			}
		};
		...
		this.showThumbIntro2D = function(scale, del)
		{
			...
			if (!FWDUtils.hasTransform2d)
			{
				...
				TweenMax.to(self, .8, {x:Math.floor(self.newX + (self.thumbWidth - scaleW)/2), y:-Math.floor(scaleH/2), delay:del, ease:Expo.easeOut});
			}
			else
			{
				self.setScale2(self.thumbScale);
		...
		this.setScale = function(scale)
		{
			...
			if (!FWDUtils.hasTransform2d)
			{
				...
				TweenMax.to(self, .8, {x:Math.floor(self.newX + (self.thumbWidth - scaleW)/2), y:-Math.floor(scaleH/2), w:scaleW, h:scaleH, ease:Quart.easeOut});
		...
		this.hide = function(del)
		{
			...
			TweenMax.to(self.imageHolderDO, .8, {x:parseInt(self.thumbWidth/2), y:parseInt(self.thumbHeight/2), w:0, h:0, delay:del, ease:Expo.easeInOut});
			...
			else
			{
				if (self.imageDO)
				{
					...
					if (self.reflCanvasDO)
					{
						TweenMax.to(self.reflCanvasDO, .8, {alpha:0, delay:del, ease:Expo.easeInOut});
					}
				}
			}
		};
	...
	{
		FWDR3DCarThumb.prototype = new FWDDisplayObject3D("div", "absolute", "visible");
	};
...
(function(window)
{
	var FWDR3DCarThumbsManager = function(data, parent)
	{
		...
		this.setIntroFinished = function()
		{
			...
			if (FWDUtils.isIEAndMoreThen9 || !FWDUtils.hasTransform3d || self.data.showDisplay2DAlways)
			{
	...
	{
		FWDR3DCarThumbsManager.prototype = new FWDDisplayObject3D("div", "relative", "visible");
	};
...
(function(window)
{
	var FWDRoyal3DCarousel = function(props)
	{
		...
		this.onThumbsManagerIntroFinish = function()
		{
			...
			self.dispatchEvent(FWDRoyal3DCarousel.CATEGORY_CHANGED);
		};
	...
	FWDRoyal3DCarousel.CATEGORY_CHANGED = "categoryChanged";
...
(function (window){
	...
	FWDUtils.isIEAndMoreThen8 = (function(){
		return FWDUtils.isIE9 || FWDUtils.isIE10 || FWDUtils.isIE11;
	}());
	...
	FWDUtils.isIEAndMoreThen9 = (function(){
		return FWDUtils.isIE10 || FWDUtils.isIE11;
	}());
